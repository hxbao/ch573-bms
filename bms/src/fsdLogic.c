...
#include "includes.h"
#if(PROJECT_ID == 5)
#undef TN_LOAD_DET_PORT
#undef TN_LOAD_DET_PIN
#undef TN_CHG_DET_PORT
#undef TN_CHG_DET_PIN
#undef TN_PREEN_PORT
#undef TN_PREEN_PIN
#undef TN_NTC0_POWER_PORT
#undef TN_NTC0_POWER_PIN
#undef TN_NTC1_POWER_PORT
#undef TN_NTC1_POWER_PIN

#define TN_LOAD_DET_PORT      GPIOA
#define TN_LOAD_DET_PIN       GPIO_PIN_0

#define TN_CHG_DET_PORT      GPIOA
#define TN_CHG_DET_PIN       GPIO_PIN_1
...
#define TN_NTC0_POWER_PIN       GPIO_PIN_6

///<NTC1 POWER
#define TN_NTC1_POWER_PORT      GPIOB
#define TN_NTC1_POWER_PIN       GPIO_PIN_6

#define TN_VDOL_PORT   GPIOB
#define TN_VDOL_PIN    GPIO_PIN_7
...
uint8_t flagOneWrieWake = 0;  //1-控制器发送激活指令标志 ，上拉之后 拉低20ms一线通信号
...
void PortA_IRQHandler(void)
{
	...
	if (TRUE == Gpio_GetIrqStatus(TN_LOAD_DET_PORT, TN_LOAD_DET_PIN))
	{
		...
		SEGGER_RTT_printf(0,"Event->LOAD Pulldown\n");
		flagIntWake |= 0x20;
	}
...
static void System_EnterLowPower(void)
{
	...
	else
	{
		...
		Gpio_EnableIrq(TN_CHG_DET_PORT, TN_CHG_DET_PIN, GpioIrqRising); //充电器接入
#if (PROJECT_ID != 2)
		Gpio_EnableIrq(TN_LOAD_DET_PORT, TN_LOAD_DET_PIN, GpioIrqFalling);//Load 接入
#endif
...
static void GpioInterruptConfig()
{
	...
	//配置中断线
	GPIO_ConfigEXTILine(GPIOA_PORT_SOURCE, GPIO_PIN_SOURCE1);  //chg-det
	...
	GPIO_ConfigEXTILine(GPIOA_PORT_SOURCE, GPIO_PIN_SOURCE10);  //PA10 SHINT
	//GPIO_ConfigEXTILine(GPIOB_PORT_SOURCE, GPIO_PIN_SOURCE12); //PB12 ALARM
	GPIO_ConfigEXTILine(GPIOA_PORT_SOURCE, GPIO_PIN_SOURCE0); //PA11 LOAD DET
	
	EXTI_InitStructure.EXTI_Line = EXTI_LINE1; //chg-det
	...
	EXTI_InitStructure.EXTI_Line = EXTI_LINE0; //LOAD DET
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling; // EXTI_Trigger_Rising;
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
	EXTI_InitPeripheral(&EXTI_InitStructure);
...
static void System_EnterLowPower(void)
{
	...
	if (afeInfo.CellVmin < SHIP_MODE_CELLV_TH) //低电压保护后
	{
		...
		NVIC_DisableIRQ(EXTI0_IRQn);   //禁止TN_WAKE_UP
	...
	else
	{
		...
		//禁止TN_WAKE_UP，ACC，SH_INT,ALARM
		NVIC_EnableIRQ(EXTI0_IRQn);		//LOAD
		NVIC_EnableIRQ(EXTI2_IRQn);		//ACC
		//NVIC_EnableIRQ(EXTI9_5_IRQn);	//通信中断
#if (AFE_CHIP_SELECT != 3)		
		NVIC_EnableIRQ(EXTI15_10_IRQn); //通信中断
#else
...
static void NiuLogic_MosHanle(void)
{
	...
	if (bsp_CheckTimer(TMR_NIULOGIC_20S) || Rtc_GetWakeUpFlag() == 0x01) //唤醒20s 时间超时,RTC 唤醒之后，最多跑一个循环就可以
	{
		...
		if (flagIntWake & 0x04)
		{
			...
			{
				//低电平检测
				if(MCU_GPIO_GetBit(TN_ONE_RX_PORT, TN_ONE_RX_PIN) == 1)
				{
					flagOneWrieWake = 0;
					break;
				}
