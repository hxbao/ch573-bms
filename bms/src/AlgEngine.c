...
static void SocOCVCalibrate(uint16_t avgCellVolt)
{
    ...
    if (avgCellVolt >= OCV_Line[0])
    {
        ...
        }else
        {
            ...
            SEGGER_RTT_printf(0,"OCV Calibrated,ocvCapdiff*1000 = %d\n",algIntnel.ocvCapdiff*1000);
        }
    ...
    else if (avgCellVolt <= OCV_Line[20])
    {
        ...
        }else
        {
            ...
            algIntnel.ocvCapdiffdivbyM = algIntnel.ocvCapdiff/(1500);
            SEGGER_RTT_printf(0,"OCV Calibrated,ocvCapdiff*1000 = %d\n",algIntnel.ocvCapdiff*1000);
        }
    ...
    if (bsp_CheckTimer(TMR_SOC_ENGINE)|| (Rtc_GetWakeUpFlag() == 0x01))
    {
                ...
                if(algIntnel.flagState == 1)
                {
                    ...
                    if((calcDsgCapacityAH < algIntnel.capacity*1.2) && (calcDsgCapacityAH > algIntnel.capacity*0.75))
                    {
                        if ((algIntnel.RecodEEPromFlag & 0x01) == 0x00)
                        {
                            ...
                            //{
                                SEGGER_RTT_printf(0,"Info->SocEngine.capacity is updated,algIntnel.capacity x1000->%d\n",(uint16_t)(calcDsgCapacityAH*1000));
        ...
        else if (algIntnel.SocState == 2) //充电状态
        {
            ...
            if (algIntnel.ChgCapacityAH >= 0.9 * algIntnel.capacity)
            {
                ...
                {
                    algEnginer.cycCount++;
                    SEGGER_RTT_printf(0,"Info->SocEngine.cycCount is updated,SocEngine.cycCount ->%d\n",algEnginer.cycCount);
...
static void SocTimerCallback(void)
{
    ...
    if (algIntnel.SocState == 1)
    {
        ...
        //在放电模式，容量差大于1%额定容量 
        //算法均应分摊到放电到soc为0的周期
        if(algIntnel.ocvCapdiff>0.005*algIntnel.capacity)
        {            
        ...
        if(algIntnel.ocvCapdiff<-0.005*algIntnel.capacity)
        {
            ddf = 0.3*temp+algIntnel.ocvCapdiffdivbyM;
            if(ddf>0)
            {
                //algIntnel.ocvCapdiff 为负值 algIntnel.ocvCapdiffdivbyM也为负值
                algIntnel.ocvCapdiff -= algIntnel.ocvCapdiffdivbyM;
                algEnginer.resCapAH_r -= algIntnel.ocvCapdiffdivbyM ;
            }
...
{
    float temp;
    static uint8_t isStartFreeRunTime = 0;
    uint32_t runTimeMs = 0;

    //1、电量积分计算
    if (algIntnel.SocState == 1)
    {
        if(isStartFreeRunTime == 0){
            isStartFreeRunTime = 1;
            bsp_StartFreeRunTime();
        }else{
            runTimeMs = bsp_GetFreeRunTime();
            SEGGER_RTT_printf(0,"runTimesMs->%d\n",runTimeMs);
            bsp_StartFreeRunTime();
             //放电容量积分
            temp = (float)runTimeMs*afeInfo.DsgCurrent / 1000000 / 3600;
    ...
    else if (algIntnel.SocState == 2)
    {
        if(isStartFreeRunTime == 0){
            ...
            temp = (float)runTimeMs*afeInfo.ChgCurrent / 1000000 / 3600;

            if((algEnginer.resCapAH_r + temp) < algIntnel.capacity)
            {
                algEnginer.resCapAH_r += temp;
            }
            algIntnel.ChgCapacityAH += temp;
