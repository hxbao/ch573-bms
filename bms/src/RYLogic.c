...
static void NiuLogic_PortInit()
{
    ...
    GPIO_SetBits(TN_LED2_PORT,TN_LED2_PIN);
}
...
static void NiuLogic_UpdateEEPROM(void)
{
	...
	//OCD2
	EEP_RegMap[13] &= ~0xF0;
	EEP_RegMap[13] |= ((uint8_t)(CmmdConfigData.OCD / 10) & 0x0F) << 4;
	//SCV
	EEP_RegMap[14] &= 0x0F;
	if(ISC_P>255 && ISC_P>350)
	{
		//最大350A
		EEP_RegMap[14] |= ((uint8_t)((ISC_P - 50) / 30) & 0x0F) << 4;
	}else
	{
