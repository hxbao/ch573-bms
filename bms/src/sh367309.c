...
#define VPRO_CON_ON()                    \
	do                                   \
	{                                    \
	  MCU_GPIO_SetBit(TN_VPRO_CON_PORT, TN_VPRO_CON_PIN); \
	} while (0);

#define VPRO_CON_OFF()                     \
	do                                     \
	{                                      \
		MCU_GPIO_ClrBit(TN_VPRO_CON_PORT, TN_VPRO_CON_PIN); \
	} while (0);
...
static void SH_iicWriteRam(uint8_t ramAddr, uint8_t ramRegVal)
{
	...
	if ((ramAddr >= 0x40) && (ramAddr <= 0x70))
	{
		...
		while(ret)
		{
			...
			if(count == 0)
			{
				SEGGER_RTT_printf(0,"sh309 write regs timerout\n");
				break;
			}
		}
	}
...
static uint8_t SH_iicReadRam(uint8_t ramAddr, uint8_t readSize, uint8_t *pdataBuf)
{
	...
	if ((ramAddr >= 0x40) && (ramAddr <= 0x70))
	{
		...
		while(ret)
		{
			...
			if(count == 0)
			{
				SEGGER_RTT_printf(0,"sh309 read regs timerout\n");
				break;
			}
		}
...
void SH_iicWriteEEPROM(uint8_t EEAddr, uint8_t eeRegVal)
{
	...
	if (EEAddr <= 0x18)
	{
		...
		while(ret)
		{
			...
			if(count == 0)
			{
				SEGGER_RTT_printf(0,"sh309 write EEPROM timerout\n");
				break;
			}
		}
		//delay1ms(35);
...
uint8_t SH_iicReadEEPROM(uint8_t EEAddr, uint8_t readSize, uint8_t *pdataBuf)
{
	...
	if (EEAddr <= 0x18)
	{
		...
		while(ret)
		{
			...
			if(count == 0)
			{
				SEGGER_RTT_printf(0,"sh309 read EEPROM timerout\n");
				break;
			}
		}
	}
